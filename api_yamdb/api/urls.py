...
urlpatterns = [
    ...
    path('v1/users/me/',
         UserMeViewSet.as_view({'get': 'retrieve', 'patch': 'update'}), name='user_me'),
    path('v1/users/<str:username>/',
         UsernameViewSet.as_view({'get': 'retrieve', 'patch': 'update', 'delete': 'destroy'}), name='username'),
    path('v1/users/',
         UsersViewSet.as_view({'get': 'list', 'post': 'create'}), name='users')
]
